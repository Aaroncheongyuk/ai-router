...
function buildModelSelectorAliases(modelKey, model) {
...
function buildModelAliasMap(config) {
...
function resolveCanonicalModelKey(config, requestedModelKey) {
  const models = config.models?.models ?? {};
...
function buildRuntimeEnv(config, providerKey, modelKey) {
...
function buildCompatNotes(runtime, providerProtocol) {
...
function resolveTargetFromModel(config, requestedModelKey) {
...
function findMatchingRole(routes, role) {
  // Exact match
  if (routes[role]) {
    return role;
  }
...
export function resolveRouteFromConfig(config, request) {
...
export async function validateLoadedRouterConfig(options = {}) {
  const config = await loadRouterConfig(options);
  validateRouterConfig(config);
  return config;
}

export async function resolveRoute(request, options = {}) {
  const config = await loadRouterConfig(options);
